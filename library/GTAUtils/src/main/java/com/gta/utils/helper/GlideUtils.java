 ...
 * [Tips]
 *
 * @author Created by  on 2016/12/28.
 * @since 1.0.0
 */
...
public abstract class GlideUtils {
     ...
     */
    public static <T> void loadImage(T context, final String url, final ImageView imageView) {
        DrawableTypeRequest<String> type = getContext(context, url);
        if (type != null) {
            type.asBitmap();
            type.placeholder(R.drawable.loading_bg_cn);   //占位符
            type.error(R.drawable.loading_fail_bg_cn);    //图片加载失败显示图片
        }
        ...
        type.listener(new GlideListener<String, GlideDrawable, String>(null, url, imageView))
                .into(imageView);
    }
     ...
     * @param imageView 要显示的Imageview
     */
    public static <T> void loadUserImage(T context, final String url, final ImageView imageView) {
        DrawableTypeRequest<String> type = getContext(context, url);
        if (type != null) {
            type.asBitmap();
            type.placeholder(R.drawable.default_head_image);   //占位符
            type.error(R.drawable.default_head_image);    //图片加载失败显示图片
        }
    ...
    public static void loadImageFormVideo(String url, ImageView imageView) {
        Glide.with(imageView.getContext()).load(url)
                .override(ConvertUtils.dp2px(150), ConvertUtils.dp2px(150))
                .placeholder(R.drawable.loading_bg_cn)
                .dontAnimate()
                .into(imageView);
    }
