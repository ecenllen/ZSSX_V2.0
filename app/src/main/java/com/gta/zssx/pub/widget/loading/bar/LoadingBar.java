package com.gta.zssx.pub.widget.loading.bar;
...
import com.gta.zssx.pub.widget.loading.factory.LoadingFactory;
import com.gta.zssx.pub.widget.loading.factory.MaterialFactory;
...
public final class LoadingBar implements ILoadingBar {
    ...
    private LoadingBar(ViewGroup parent, LoadingFactory factory, String message) {
        mParent = parent;
    ...
    public LoadingBar setOnReloadButtonListener(View.OnClickListener listener) {
        if(mView != null) {
            Button btReload = (Button) mView.findViewById(R.id.bt_reload);
            btReload.setOnClickListener(listener);
        }
        return this;
    }
    ...
    public static LoadingBar make(View parent, String message) {
        return make(parent, LoadingConfig.getLoadingFactory(), message);
    }
    ...
    public static LoadingBar make(View parent, LoadingFactory factory, String message) {
        //如果已经有Loading在显示了,则先把它移除掉
        if (LOADINGBARS.containsKey(parent)) {
            LoadingBar loadingBar = LOADINGBARS.get(parent);
            if(loadingBar.mParent != null)
                loadingBar.mParent.removeView(loadingBar.mView);
        }
        LoadingBar newLoadingBar = new LoadingBar(findSuitableParent(parent), factory, message);
