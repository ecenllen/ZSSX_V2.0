...
public class CourseSchedulePresenter extends BasePresenter<CourseScheduleView> {
    ...
    public void getSchedule(String uid, String date, String semesterId, int flag, String classId) {

        if (getView() == null)
            return;
        getView().showLoadingDialog();
        mCompositeSubscription.add(AdjustDataManager.getSchedule(uid, date, semesterId, flag, classId)
                .subscribe(new Subscriber<ScheduleBean>() {
                    @Override
                    ...
                    public void onNext(ScheduleBean scheduleBean) {
                        getView().showResult(scheduleBean);
                    }
                }));
    }
