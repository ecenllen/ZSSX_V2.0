 ...
 * @since 2.0.0
 */
public class ClassroomCourseAddAdapter extends RecyclerView.Adapter {
    private List<DetailItemShowBean.CourseInfoBean> mCourseInfoBeenList;
    private Listener listener;
    private Context mContext;
    private String mSearchString;
    private int State;//区分是  搜索课程 还是 我的课程  1 搜索  2 我的

    public ClassroomCourseAddAdapter (Context context) {
        mContext = context;
    }
    ...
    public void setCourseInfoBeenList (List<DetailItemShowBean.CourseInfoBean> mCourseInfoBeenList) {
        this.mCourseInfoBeenList = mCourseInfoBeenList;
    }
    ...
    public void onBindViewHolder (RecyclerView.ViewHolder holder, int position) {
        ...
        courseHolder.mTextView.setOnClickListener (v -> listener.itemClickListener (courseInfoBean));
        if (State == 1) {
            courseHolder.mTextView.setText (getSpannableString (courseName));
        } else {
            mSearchString = courseName;
            courseHolder.mTextView.setText (getSpannableString (courseName));
        }
    }
    ...
    private SpannableStringBuilder getSpannableString (String string) {
        ...
        if (!allLowerCaseStr.contains (mSearchStr)) {
            ...
            styleAll.setSpan (new ForegroundColorSpan (ContextCompat.getColor (mContext, R.color.word_color_666666)), 0, string.length (), Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
            return styleAll;
        }
        //关键字和结果一样的时候
        if (allLowerCaseStr.equals (mSearchStr)) {
            SpannableStringBuilder styleAll = new SpannableStringBuilder (string);
            styleAll.setSpan (new ForegroundColorSpan (ContextCompat.getColor (mContext, R.color.main_color)), 0, string.length (), Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
            return styleAll;
        }
        //含有关键字且不等于关键字的时候
        ...
        if (keyWordStart == 0 && keyWordEnd != string.length ()) {
            ...
            styleAll.setSpan (new ForegroundColorSpan (ContextCompat.getColor (mContext, R.color.main_color)), keyWordStart, keyWordEnd, Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
            ...
            if (subString.length () >= mSearchStr.length ()) {
                ...
                //否则直接赋予颜色值
                sub.setSpan (new ForegroundColorSpan (ContextCompat.getColor (mContext, R.color.word_color_666666)), 0, sub.length (), Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
                styleAll.append (sub);
            }
            return styleAll;
            ...
            styleAll.setSpan (new ForegroundColorSpan (ContextCompat.getColor (mContext, R.color.word_color_666666)), normalWordStart, normalWordEnd, Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
            styleAll.setSpan (new ForegroundColorSpan (ContextCompat.getColor (mContext, R.color.main_color)), keyWordStart, keyWordEnd, Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
            return styleAll;
        } else {
            ...
            styleAll.setSpan (new ForegroundColorSpan (ContextCompat.getColor (mContext, R.color.word_color_666666)), normalWordStart1, normalWordEnd1, Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
            styleAll.setSpan (new ForegroundColorSpan (ContextCompat.getColor (mContext, R.color.main_color)), keyWordStart, keyWordEnd, Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
