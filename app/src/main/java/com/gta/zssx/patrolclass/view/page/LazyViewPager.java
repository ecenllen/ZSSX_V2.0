 ...
 * 只修改了原始ViewPager的DEFAULT_OFFSCREEN_PAGES属性,让其加载第一页时不自动加载第二页
 */
public class LazyViewPager extends ViewGroup {
     ...
     * is fully in view and no animation is in base_loading_progress.
     */
...
//            mAdapter.registerDataSetObserver(mObserver);
    ...
    private void recomputeScrollPosition(int width, int oldWidth, int margin, int oldMargin) {
        ...
        if (oldWidth > 0) {
            ...
            if (!mScroller.isFinished()) {
                // We now return to your regularly scheduled scroll, already in base_loading_progress.
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        if (mFakeDragging) {
            // A fake drag is in base_loading_progress already, ignore this real one
     ...
     * is already in base_loading_progress, this method will return false.
     *
    ...
    public void endFakeDrag() {
        if (!mFakeDragging) {
            throw new IllegalStateException("No fake drag in base_loading_progress. Call beginFakeDrag first.");
        }
    ...
    /**
     * Returns true if a fake drag is in base_loading_progress.
     *
